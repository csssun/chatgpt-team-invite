...
@app.route('/api/admin/team-accounts/<int:account_id>/codes', methods=['DELETE'])
@admin_required
def delete_team_account_codes(account_id):
    """删除指定车位下的邀请码
    - 默认仅删除未使用的邀请码 used=0
    - ?include_used=1 则连已使用的邀请码也删除（会清除使用记录）
    """
    include_used = str(request.args.get('include_used', '0')).lower() in ('1', 'true', 'yes')
    conn = get_db()

    # 检查车位是否存在
    acc = conn.execute('SELECT id FROM team_accounts WHERE id = ?', (account_id,)).fetchone()
    if not acc:
        conn.close()
        return jsonify({'error': '车账号不存在'}), 404

    if include_used:
        cur = conn.execute('DELETE FROM invite_codes WHERE team_account_id = ?', (account_id,))
    else:
        cur = conn.execute('DELETE FROM invite_codes WHERE team_account_id = ? AND used = 0', (account_id,))

    deleted = cur.rowcount if cur.rowcount is not None else 0
    conn.commit()
    conn.close()
    return jsonify({'status': 'deleted', 'deleted': deleted, 'includeUsed': include_used})
...
def delete_team_account(account_id):
    ...
    if count > 0 and not force:
        conn.close()
        return jsonify({'error': '该车位下有邀请码，无法删除', 'codeCount': count, 'hint': '可先清空邀请码后再删除，或使用 force=1 强制删除'}), 400

    # force 模式：先清空邀请码，再删除车位
    if count > 0 and force:
        conn.execute('DELETE FROM invite_codes WHERE team_account_id = ?', (account_id,))
